...
def BFS(start, goal):
    ...
    # Queue ordered by a min weighted distance
    queue = {vertexList[start].start_dist: [(current,  path)]}
    in_queue = [start]
    ...
    while queue != []:
        ...
        for nxt in current.adjacent:
            ...
            # If the next adjacent node is the goal, add it to the path.
            if nxt == goal:
                path.append(nxt)
                return path
            ...
            # If is is not, add it to the queue with the weighted distance. Only add nodes that have not already been added.
                ...
                if nxt not in in_queue:
                    ...
                    shortest_dist = vertexList[nxt].start_dist
